...
import 'package:smavy/src/pages/historial/historial_controller.dart';
...
class _HistorialPageState extends State<HistorialPage> {
  final HistorialController _con = HistorialController();
  ...
  Widget build(BuildContext context) {
    ...
    Scaffold(
      ...
      body: Column(
        children: [
          (_con.datosCargados && _con.travelHistoryList.isNotEmpty)?
          SizedBox(
            height: MediaQuery.of(context).size.height*0.85,
            child: ListView.builder(
              shrinkWrap: true,
              physics: BouncingScrollPhysics(),
              itemCount: _con.travelHistoryList.length,
              itemBuilder: (BuildContext context, int index) =>
                _buildList(_con.travelHistoryList[index], index)
            ),
          ):
          SizedBox()
        ],
  ...
  Widget _buildList(TravelHistory data, int index) {
    return ExpansionTile(
      ...
      title: Row(
        children: [
          Text(
            _con.readTimestamp(data.timestamp),
